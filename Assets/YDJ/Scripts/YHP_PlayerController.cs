...
public class YHP_PlayerController : MonoBehaviour
{
    ...
    private void OnMove( InputValue value )
    {
        if ( cameraSwitch.IsPlayer1Active && !onIce && inputKey )
        {
            ...
            if(moveDir != transform.forward && playerInput.actions["Move"].WasPressedThisFrame() && !grabOn )
            {
    ...
    public void OnPull( InputValue value )
    {
        ...
        Collider [] mirror = Physics.OverlapSphere(transform.position + transform.forward, 1f, LayerMask.GetMask("Mirror"));
        if ( mirror.Length > 0 )
        {
            Debug.Log(mirror);
            return;
    ...
    public void MoveFunc()
    {
        ...
        else if ( !moveOn && grabOn && moveDir.magnitude > 0 )
        {
            ...
            if ( grabHit.collider != null )
            {
                ...
                if ( Physics.Raycast(transform.position + new Vector3(0, 1, 0), -transform.forward, out RaycastHit hit, 1.5f, wall| obstacleLayer) )
                {
                ...
                else
                {
                    ...
                    foreach ( Collider isCollider in isBlank )
                    {
                        ...
                        if ( obstacleLayer.Contain(isCollider.gameObject.layer) && !mirrorHolding )
                        {
                            ...
                            float time = 0;
                            LayerMask layer = obstacleLayer | wall | crystal;   // 뒤쪽에 뭔가 있으면 못밀도록 박스캐스트로 확인
                            ...
                            {
                                if (Physics.Raycast(holder.transform.position,moveDir,2f,LayerMask.GetMask("Mirror")))      //거울이 있으면 거울을 통해 전송시키도록 
                                {
                                    StartCoroutine(PushObstacle(true));
                                }
                                else        //그것도 아니라면 못 밀도록
                                {
                            ...
                            {
                                Debug.Log("false obsPush");
                                StartCoroutine(PushObstacle(false));
                            }
                            moveSpeed = 4;
                            IEnumerator PushObstacle(bool onMirror)
                            {
                                if ( startPush )
                                    yield break;
                                ...
                                if ( onMirror )
                                {
                                    yield return StartCoroutine(TransmissionCheck());
                                    if ( !moveOn )
                                        yield break;
                                }
                                else
                                {
                                    Debug.Log("바닥거울로 전송");
                                    Collider [] col = Physics.OverlapSphere(obsTargetPos, 1f, mirror);
                                    if(col.Length > 0 )
                                    {
                                        yield return StartCoroutine(TransmissionCheck());
                                        if ( !moveOn )
                                ...
                                while ( time < 2 )
                                {
                                    if ( Physics.Raycast(isCollider.gameObject.transform.position + new Vector3(0, 0.5f, 0), moveDirValue, out RaycastHit notThis, 1f, layer) 
                                                                                          && notThis.collider.gameObject != isCollider.gameObject &&!onMirror )
                                    {
                                        animator.SetBool("Push", false);
                                        moveOn = false;
                                        startPush = false;
                                  ...
                                  //  isCollider.GetComponent<Rigidbody>().MovePosition(Vector3.Lerp(startObs, obsTPos, time / 2));
                                  isCollider.transform.position = Vector3.Lerp(startObs, obsTPos, time / 2);
                                    yield return null;
                                }
                                Manager.game.StepAction++;
                                moveOn = false;
                                ...
                                startPush = false;
                                if(onMirror)
                                StartCoroutine(mirror1.MirrorInObstacleWall(isCollider.gameObject));


                                IEnumerator TransmissionCheck()
                                {
                                    RaycastHit [] walls = Physics.RaycastAll(mirror2.transform.position, mirror2.transform.up, 10f, wall);
                                    if ( walls.Length > 0 )
                                    {
                                        Debug.Log("전송시킬 위치 위에 무엇인가 있다");
                                        moveOn = false;
                                        yield break;
                                    }
                                    else
                                    {
                                        Debug.Log($"전송 위치에 아무벽도 없다 {walls.Length}");
                                    }
                                }
                            }
                        }
    ...
    private void Hold()
    {
        ...
        {
            mirror1 = mirrorObject.GetComponent<Mirror1>();
            mirror2 = mirror1.Mirror2;
            mirror1Image = mirror1.mirrorImage;
        }
    ...
    private void UnHold()
    {
        ...
        if ( mirrorObject != null )
        {
            Mirror1 mirror = mirrorObject.GetComponent<Mirror1>();
